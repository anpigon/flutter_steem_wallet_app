...
class PostsView extends GetView<PostsController> {
  ...
  PostsView(this.subscription) {
    Get.lazyPut(
      () => PostsController(subscription),
      tag: subscription.tag,
      // fenix: true, // 생성된 인스턴스를 메모리에 계속 유지
    );
  }

  @override
  Widget build(BuildContext context) {
    final controller = Get.find<PostsController>(tag: subscription.tag);
    ...
    return Obx(() {
      if (controller.data.isEmpty) {
        return Center(
          ...
          children: [
            Text('아무것도 없다.'),
            ElevatedButton(onPressed: controller.loadPosts, child: Text('재시도')),
          ],
        ));
      }
